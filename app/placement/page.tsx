...
const gradeBands: { value: PlacementGradeBand; label: string }[] = [
  { value: 'g1-3', label: '초 1–3' },
  { value: 'g4-6', label: '초 4–6' },
  { value: 'jh1-3', label: '중 1–3' },
];
        ...
        <section className="flex flex-1 flex-col justify-center py-8 sm:py-12">
          {step === 'intro' && (
            <div className="mx-auto w-full max-w-2xl text-center">
              ...
              <h1 className="mt-4 text-4xl font-black leading-tight sm:text-6xl">첫 장면으로<br />내 시작점 찾기</h1>
              <p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-zinc-400 sm:text-lg">
                Sing 2 장면을 보고, 따라 말하고, 소리 내어 읽어요. 학년보다 실제 수행을 보고 시작 단계를 추천합니다.
              </p>
              ...
              <div className="mx-auto mt-8 max-w-lg rounded-2xl border border-white/10 bg-[#201e1e] p-5 text-left">
                ...
                <p className="mt-3 text-xs leading-relaxed text-zinc-500">학년은 참고만 합니다. 최종 추천은 말하기·읽기·이해 수행으로 정해져요.</p>
              </div>
          ...
          {step === 'watch' && (
            <div className="mx-auto w-full max-w-2xl">
              ...
              <p className="mt-3 text-zinc-400">잠시 뒤 이 장면의 소리와 내용을 확인할게요.</p>
          ...
          {currentVoiceTask && (
            <div className="mx-auto w-full max-w-2xl text-center">
              <p className="text-sm font-black text-[#60D96C]">
                {step === 'reading' ? '4 · 소리 내어 읽기' : step === 'easy' ? '2 · 짧게 따라 말하기' : '3 · 길게 따라 말하기'}
              </p>
              <h1 className="mt-2 text-3xl font-black sm:text-5xl">
                {step === 'reading' ? '문장을 보고 읽어보세요' : '소리를 듣고 그대로 말해보세요'}
              </h1>
              {step === 'reading' ? (
              ...
              <p className="mt-5 text-sm text-zinc-500">
                {step === 'reading' ? '준비되면 녹음을 시작하세요.' : '필요하면 여러 번 들어도 괜찮아요.'}
              </p>
              <button
                type="button"
          ...
          {step === 'result' && result && (
            <div className="mx-auto w-full max-w-2xl text-center">
              ...
              <div className="mt-7 rounded-2xl border border-white/10 bg-[#201e1e] p-5 text-left">
                <p className="text-sm font-black text-[#60D96C]">이렇게 판단했어요</p>
              ...
              {result.level === 'studio-ready' && (
                <p className="mt-4 text-sm text-zinc-500">Studio 확정은 선생님과 독립 리텔링을 한 번 더 확인한 뒤 결정해요.</p>
              )}
              <button
                type="button"
      ...
      {activeCoach && (
        <div className="fixed inset-0 z-[100]">
          <AiCoachPanel
            ...
            promptText={activeCoach === 'reading' ? '문장을 자연스럽게 소리 내어 읽어보세요.' : undefined}
            targetPreview={activeCoach === 'reading' ? VOICE_TASKS.reading.text : undefined}
            continueLabel="다음으로"
            fallbackLabel="음성 분석 없이 계속하기"
            showScore={false}
            onResult={() => undefined}
